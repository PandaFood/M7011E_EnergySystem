/*
* Not made by us.
* Taken from https://github.com/josephg/noisejs/blob/master/perlin.js
*
*
...
/**
 * Object for calculating a WindMap
 */
const Noise = {
	baseNoiseMap: [],
	noiseMap: [],
	...
	updateWindMap: function(seedVal) {
		...
		const sin1 = Math.sin((time)*1/71+15);
		const sin2 = Math.sin((time)*1/57+6);
		const sin3 = Math.sin((time)*1/36+3);
		const sin4 = Math.sin((time)*1/15);
	...
	generateWindMap: function(seedVal) {
		seed(seedVal);

		for (let y = 0; y < this.size; y++) {
			this.baseNoiseMap.push([]);
			this.noiseMap.push([]);

			for (let x = 0; x < this.size; x++) {
				this.baseNoiseMap[y].push(0);
				this.noiseMap[y].push(0);
